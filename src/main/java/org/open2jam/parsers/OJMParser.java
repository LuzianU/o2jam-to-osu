...
public class OJMParser {
    ...
    public static HashMap<Integer, SampleData> parseFile(File file) {
        ...
        try {
            ...
            Logger.global.log(Level.WARNING, "IO exception on file {0} : {1}",
        ...
        for (int i = 0; i < sample_count; i++) {
            ...
            switch (encryption_flag) {
                case 0:
                    break; // Let it pass
                case 16:
                    M30_xor(sample_data, mask_nami);
                    break;
                case 32:
                    M30_xor(sample_data, mask_0412);
                    break;
                default:
                    Logger.global.log(Level.WARNING, "Unknown encryption flag({0}) !", encryption_flag);
            }

            SampleData audioData = new SampleData(new ByteArrayInputStream(sample_data), SampleData.Type.OGG,
                    sample_name);
            int value = ref;
            if (codec_code == 0) {
                value = 1000 + ref;
            } else if (codec_code != 5) {
                Logger.global.log(Level.WARNING, "Unknown codec code [{0}] on OJM : {1}",
                        new Object[] { codec_code, file.getName() });
            }
    ...
    private static HashMap<Integer, SampleData> parseOMC(RandomAccessFile f, boolean decrypt) throws IOException {
        ...
        while (file_offset < ogg_start) // WAV data
        {
            ...
            SampleData.WAVHeader header = new SampleData.WAVHeader(audio_format, num_channels, sample_rate, bit_rate,
                    block_align, bits_per_sample, data, chunk_size);
