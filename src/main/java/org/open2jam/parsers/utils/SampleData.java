...
public class SampleData {
	...
	public void copyToFolder(File directory) throws IOException {
		String filename = this.filename.replaceAll("[/:\"*?<>|]", "").trim();
		File f = new File(directory, filename);
		if (!f.exists()) {
			FileOutputStream out = new FileOutputStream(f);

			// WHY IS THE HEADER BEING WRITTEN TWICE!!!!!????
			// if (this.type == Type.WAV_NO_HEADER)
			// out.write(WAVHeader.writeHeader(this.wavHeader));
	...
	public static class WAVHeader {
		...
		public WAVHeader(short audio_format, short num_channels, int sample_rate, int bit_rate, short block_align,
				short bits_per_sample, int data, int chunk_size) {
